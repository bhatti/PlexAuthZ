...
func testCRUDPrincipals(
	...
	for _, next := range res {
		...
		// THEN it should not fail
		require.NoError(t, err)
		// WHEN Deleting a principal without org-id or id should fail
		require.Error(t, authService.DeletePrincipal(ctx, "", next.Id))
		require.Error(t, authService.DeletePrincipal(ctx, org.Id, ""))
	}
}
