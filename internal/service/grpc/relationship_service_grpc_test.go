...
func testCRUDRelationships(
	...
	for _, next := range res {
		...
		require.Equal(t, 2, len(relationship.Attributes))
		// WHEN deleting a relationship
		err := authService.DeleteRelationship(ctx, org.Id, org.Namespaces[0], next.Id)
		// THEN it should not fail
		require.NoError(t, err)
		// WHEN deleting a relationship without org-id, namespace or id should fail
		require.Error(t, authService.DeleteRelationship(ctx, "", org.Namespaces[0], next.Id))
		require.Error(t, authService.DeleteRelationship(ctx, org.Id, "", next.Id))
		require.Error(t, authService.DeleteRelationship(ctx, org.Id, org.Namespaces[0], ""))
	}
}
